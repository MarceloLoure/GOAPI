...
import (
	...
	"github.com/MarceloLoure/GOAPI/internal/entity"
)
...
func (pd *ProductDB) GetProduct(id string) (*entity.Product, error) {
	var product entity.Product
	err := pd.db.QueryRow("SELECT id, name, price, category_id, image_url FROM products WHERE id = ?", id).Scan(&product.ID, &product.Price, &product.CategoryID, &product.ImageURL)
